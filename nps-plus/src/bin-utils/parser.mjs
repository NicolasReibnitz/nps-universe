...
import { preloadModule, loadConfig, initialize, help, specificHelpScript } from './index.mjs';
import getCompletionScripts from './autocomplete-get-scripts.mjs';

const log = getLogger();
export default parse;

async function parse(rawArgv) {
	...
	const psConfig = await getPSConfig(parsedArgv);
	...
	function showHelp(specifiedScripts) {
		...
		const commandIsHelp = isEqual(specifiedScripts[0], 'help') && !hasHelpScript;
		const shouldShowSpecificScriptHelp = commandIsHelp && specifiedScripts.length > 1;
