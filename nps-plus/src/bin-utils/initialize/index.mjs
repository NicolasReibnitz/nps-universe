...
// import { isPlainObject, camelCase, set, each, includes, startsWith } from 'lodash';
...
import stringifyObject from './stringify-object.mjs';
	...
	const defaultedScripts = Object.keys(scripts)
		...
		.reduce((obj, scriptKey) => {
			...
			if (scriptKey.indexOf('start') === 0) {
				defaultDeepKey = ['default', ...keyParts.slice(1, keyParts.length), 'default'].join('.');
			}
			let script = scripts[scriptKey];
			if (!isKeyScriptHook) {
				const { preHook, postHook } = getPrePostHooks(scripts, scriptKey, deepKey);
