...
import runNPS from './helpers/run-nps.mjs';

const fixturesPath = resolve(__dirname, './fixtures');

test('with config with default script', () => snapshot('-c ./package-scripts-with-default.js'));

test('with a missing config', () => snapshot('-c ./something-that-does-not-exist.js'));

test('with --silent', () => snapshot('test --silent'));

test('with --require', () => snapshot('--config ./es6-package-scripts.js --require babel-register log'));

test('with --get-yargs-completions', () => snapshot('--config ./package-scripts.js --get-yargs-completions li'));
